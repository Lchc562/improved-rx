...
def fun(X, key0, key1, id=0, type=0, pairs=36):
    ...
    # print(cipher_01)
    # 改变密钥，使对应位置比特错误
    print("掩码前密钥", key0, key1, key0.shape,mask.shape)
    key0 = key0 ^ masks
    key1 = key1 ^ masks
    ...
    # data 14
    X = simeck.RXDR_convert_to_bits([ctr_diff1, ctr_diff2], 2)
...
def test_bits_sensitivity(n=10 ** 7, nr=7, k=1, diff=(0x0, 0x0004), folder='./bits_sensitive_res/', type=0, pairs=36):
    acc = np.zeros(block_size + 1)
    X, Y, demo, key0, key1 = simeck.make_data_keybst_pairs(n=n, nr=nr - 1, k=k, diff=diff,pairs=pairs)
    net = load_model('./simeck_model/simeck32_data27_pairs36_1_4_r13_depth10_acc_0.9999504089355469.h5')
    ...
    for i in range(block_size):
        ...
        masked_X = fun(demo, key0, key1, id=i, type=type, pairs=36)
...
for i in range(2):
    test_bits_sensitivity(n=10 ** 6, nr=14, k=1, diff=(0x0, 0x4), folder=folder, type=i, pairs=36)
