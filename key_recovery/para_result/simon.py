...
def rol_expand_keys(key, rounds, k=15):
    k_tmp = deque(key)  # 创建一个双向队列
    keys = []
    for i in range(rounds):
        rs_3 = ((k_tmp[0] << (WORD_SIZE() - 3)) + (k_tmp[0] >> 3)) & MOD_MASK
...
def make_datas_pairs(n, nr, pairs=0, k=15, diff=(0x0, 0x3)):
    check_vector()
    Y = np.frombuffer(urandom(n), dtype=np.uint8)
    Y = Y & 1
    # Y = np.ones(n)
    ...
    keys = np.array([64258, 20420, 24904, 57508])
    key1 = rol(keys, k)
    key1[3] = key1[3] ^ diff[1]

    ks = expand_keys(keys, nr)
    ks1 = expand_keys(key1, nr)
    ...
    ctdata0l, ctdata0r = encryption((plain0l, plain0r), ks)
    ctdata1l, ctdata1r = encryption((plain1l, plain1r), ks1)
    # ctdata0l, ctdata0r = rol(ctdata0l, k), rol(ctdata0r, k)
    print("encrypt_over")
    del plain0l, plain0r, plain1l, plain1r

    # 生成部分解密1/2轮数据
    ct_diff_0111 = rol(ctdata0l, k) ^ ctdata1l
    ct_diff_0212 = rol(ctdata0r, k) ^ ctdata1r
    # print(ct_diff_0111, ct_diff_0212,Y1)
    ctr_diff1 = make_diff_decry([rol(ctdata0l, k), rol(ctdata0r, k), ctdata1l, ctdata1r], 1)
    ctr_diff2 = make_diff_decry([rol(ctdata0l, k), rol(ctdata0r, k), ctdata1l, ctdata1r], 2)

    # X = RXDR_convert_to_bits([ct_diff_0111, ct_diff_0212, rol(ctdata0l,k), rol(ctdata0r,k),
    #                           ctdata1l, ctdata1r, ctr_diff1, ctr_diff2], 8)
    # X = RXDR_convert_to_bits([ct_diff_0111, ct_diff_0212, ctdata0l, ctdata0r,
    #                           ctdata1l, ctdata1r, ctr_diff1, ctr_diff2], 8)
    X = RXDR_convert_to_bits([ct_diff_0212, ctr_diff1, ctr_diff2], 3)
    # X = RXDR_convert_to_bits([ctdata0r, ctdata1r, ctr_diff1], 3)
    print("convert_to_binary_over")
    # X = convert_to_binary([ctdata0l, ctdata0r, ctdata1l, ctdata1r])

    X = X.reshape((pairs, n, WORD_SIZE() * 3)).transpose((1, 0, 2))
    X = X.reshape(n, 1, -1)
    X = np.squeeze(X)
    print("data_over")
    return X, Y
...
# make_datas_pairs(2, 6, pairs=2, k=4, diff=(0x0, 0x22))
...
def make_datas_demo(n, rounds, diff2=(0, 0x0006), k2=1, k=15, diff=(0x0, 0x3)):
    ...
    # del plain_41, plain_52, plain_51, plain_52
    gc.collect()
    ...
    #                           ctr_diff1, ctr_diff2], 10)
    # data18
    key_diff = [0x3126, 0xaea4, 0x5e20]
    # value1,value2 = np.uint16(key_diff[0]), np.uint16(key_diff[1])
    # value1,value2 = np.full(n,value1), np.full(n,value2)
    ctr_key_diff1 = ctr_diff1 ^ key_diff[0]
    ctr_key_diff2 = ctr_diff2 ^ key_diff[1]
    # print(ctr_key_diff1,ctr_diff1,value1)
    # print(value1,cipher_01)
    # X = RXDR_convert_to_bits([ct_diff_0111, ct_diff_0212, rol(cipher_01,k), rol(cipher_02,k),
    #                           cipher_11, cipher_12, ctr_diff1, value1, ctr_diff2, value2], 10)
    # X = RXDR_convert_to_bits([ct_diff_0111, ct_diff_0212, rol(cipher_01,k), rol(cipher_02,k),
    #                           cipher_11, cipher_12, ctr_diff1, ctr_key_diff1, ctr_diff2, ctr_key_diff2], 10)
    # X = RXDR_convert_to_bits([ct_diff_0111, rol(cipher_01,k),cipher_11], 3)
    # X = RXDR_convert_to_bits([ct_diff_0212, rol(cipher_02,k),cipher_12, ctr_diff1 , ctr_diff2], 5)
    # X = RXDR_convert_to_bits([rol(cipher_02, k), cipher_12, ctr_diff1, ctr_diff2], 4)
    # X = RXDR_convert_to_bits([rol(cipher_02, k), cipher_12, ctr_diff2], 3)
    # X = RXDR_convert_to_bits([rol(cipher_02, k), cipher_12], 2)
    X = RXDR_convert_to_bits([ctr_diff1], 1)
    return X, Y

# X, Y = make_datas_demo(3, 6, k=1, diff=(0x0, 0x6))
...
#     # 遍历 input 中的元素，根据 i%4 的值放入不同的 para_result 行
#     for i in range(4 * n):
#         row = i % 4# 计算要放入的 para_result 行
#         col = i // 4  # 计算要放入的 para_result 列
