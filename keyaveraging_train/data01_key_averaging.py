...
def key_average(ct0a, ct1a, ct0b, ct1b, keys, net, k=15, diff=(0x0, 0x3)):
    ...
    X = simon.data14_generate([pt0a, pt1a, pt0b, pt1b], k=k, if_rotation=1)
...
def make_trainset_with_teacher(n, net, nr=12, k=15, diff=(0x0, 0x3), num_keys=1000, keys=None, counts=16):
    ...
    # X = simon.convert_to_bits(np.concatenate(X, axis=1))  # X进行了分割，维度变成了三维,拼接之后降了一个维度,数据还没有对齐
    X = simon.data14_generate(np.concatenate(X, axis=1),k=k,if_rotation=1)
    X_f1_tmp = simon.data14_generate(X_f1, k=k, if_rotation=0)  # 只有两个维度
    with strategy.scope():
        Y_f1 = net.predict(X_f1_tmp, batch_size=2 ** 18).flatten()
    print("genarate over")
    return X, X_f1_tmp, Y, Y_f1, Z
...
def train_speck_distinguisher(num_train, num_eval, num_epochs, num_keys=100, num_rounds=7, depth=1, batch_size=10 ** 5,
    ...
    model_teacher = load_model('data14_diff1_15_03_r12_depth10_0.6270.h5', compile=False)
    ...
    np.savez('x_xf1_y_yf1_z_r12_array18', x=X, x_f1=X_f1, y=Y, y_f1=Y_f1, z=Z)
    ...
    np.savez('x_xf1_y_yf1_z_r12_eval_array18', x=X_eval, x_f1=X_eval_f1, y=Y_eval, y_f1=Y_f1_eval, z=Z_eval)
...
train_speck_distinguisher(num_train=2 ** 10, num_eval=2 ** 7, batch_size=2 ** 15, num_keys=2 ** 16, num_epochs=60,
                          num_rounds=13, depth=5, num_block=8, k=15, diff=(0x0, 0x0003), counts1=2 ** 2, counts2=2 ** 0)
